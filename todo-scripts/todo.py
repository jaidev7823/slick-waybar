...
GOOGLE_SYNC_CONFIG_FILE = DATA_DIR / "google_sync.json"
...
def _append_history_events(events: List[Dict[str, Any]]) -> None:
    if not events:
        return
    ensure_data_dir()
...
def _log_sync_error(message: str) -> None:
    ensure_data_dir()
    with (DATA_DIR / "sync_errors.log").open("a") as f:
        f.write(f"[{now_utc_iso()}] {message}\n")


def _load_google_sync_config() -> Dict[str, Any]:
    if not GOOGLE_SYNC_CONFIG_FILE.exists():
        return {}
    try:
        raw = json.loads(GOOGLE_SYNC_CONFIG_FILE.read_text())
...
def _load_sync_state() -> Dict[str, Any]:
    if not SYNC_STATE_FILE.exists():
        return {"history_event_hashes": []}
    try:
        raw = json.loads(SYNC_STATE_FILE.read_text())
        if isinstance(raw, dict):
            raw.setdefault("history_event_hashes", [])
            if isinstance(raw["history_event_hashes"], list):
                return raw
    except Exception:
        pass
    return {"history_event_hashes": []}
...
def _google_sync_settings() -> Dict[str, Any]:
    cfg = _load_google_sync_config()
    enabled_env = (os.getenv("TODO_GOOGLE_SYNC", "") or "").strip().lower()
    enabled = cfg.get("enabled", False) or enabled_env in {"1", "true", "yes", "on"}

    client_secret = cfg.get(
        "client_secret_file",
        str(DATA_DIR / "google_client_secret.json"),
    )
    token_file = cfg.get("token_file", str(DATA_DIR / "google_token.json"))
    tasklist_id = cfg.get("tasklist_id", "@default")
    return {
        "enabled": enabled,
        "client_secret_file": Path(client_secret).expanduser(),
        "token_file": Path(token_file).expanduser(),
        "tasklist_id": str(tasklist_id),
    }


def _get_google_tasks_service() -> Tuple[Optional[Any], Optional[str]]:
    settings = _google_sync_settings()
    ...
    if token_file.exists():
        try:
            creds = Credentials.from_authorized_user_file(str(token_file), GOOGLE_TASKS_SCOPE)
        except Exception:
            creds = None
    ...
    try:
        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                ...
                flow = InstalledAppFlow.from_client_secrets_file(
                    str(client_secret_file), GOOGLE_TASKS_SCOPE
                )
                try:
        ...
        return None, f"Google auth/sync failed: {exc}"


def sync_completed_tasks_to_google(tasks: List[Dict[str, Any]]) -> int:
    if not tasks:
        return 0
    settings = _google_sync_settings()
    if not settings.get("enabled"):
    ...
    service, err = _get_google_tasks_service()
    if err:
        _log_sync_error(err)
        return 0
    if service is None:
    ...
    synced = 0
    for task in tasks:
        completed_at = task.get("completed_at") or now_utc_iso()
        body = {
        ...
        try:
            ...
            _log_sync_error(f"Failed to sync task '{body['title']}': {exc}")

    return synced


def sync_history_to_google(limit: int = 200) -> int:
    ensure_data_dir()
    ...
    seen = set(state.get("history_event_hashes", []))
    pending_tasks: List[Dict[str, Any]] = []
    new_hashes: List[str] = []

    for row in lines[-max(1, limit):]:
        try:
            event = json.loads(row)
        except Exception:
            continue
        if event.get("type") != "completed_archived":
            continue
        event_hash = hashlib.sha256(row.encode("utf-8")).hexdigest()
    ...
    synced = sync_completed_tasks_to_google(pending_tasks)
    if synced <= 0:
        return 0

    state_hashes = state.get("history_event_hashes", [])
    if not isinstance(state_hashes, list):
        state_hashes = []
    state_hashes.extend(new_hashes[:synced])
    # Keep file bounded
    state["history_event_hashes"] = state_hashes[-5000:]
...
def clear_completed(state: Dict[str, Any]) -> None:
    completed = [t for t in state.get("tasks", []) if t.get("done")]
    if not completed:
        return
        ...
        events.append(
            {
                ...
                "task": {
                    ...
                    "completed_at": task.get("completed_at") or archived_at,
                },
            }
        )
    _append_history_events(events)
    sync_completed_tasks_to_google(completed)
...
def print_history(limit: int = 30) -> None:
    ...
    for row in selected:
        try:
            ...
            continue
        task = event.get("task", {})
        when = task.get("completed_at") or event.get("archived_at") or "unknown-time"
        pr = task.get("priority", "?")
        title = task.get("title", "")
        print(f"{when} | P{pr} | {title}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sync-limit", type=int, default=200, help="History rows scanned for sync")
...
def main() -> int:
    ...
    elif args.action == "sync-history":
        synced = sync_history_to_google(args.sync_limit)
        print(f"Synced {synced} task(s) from history.")
